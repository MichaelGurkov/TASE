...
control_treat_df = map(delisted_comps_list,
...
control_treat_df = control_treat_df %>% 
...
temp = df %>% 
  select(tase_id, date, total_assets, market_cap) %>% 
  filter(!is.na(date)) %>% 
  select(-date) %>% 
  mutate(tase_id = as.character(tase_id)) %>% 
  mutate(across(where(is.numeric()), ~. * 10 ^(-3)))
  inner_join(control_treat_df %>%
              select(-diff) %>% 
