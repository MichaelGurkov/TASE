...
matching_table_by_size = match_control_group(ipo_comps = ipo_comps,
                           data_df = df,
...
delisted_comps_filepath = paste0(Sys.getenv("USERPROFILE"),
                                 "\\OneDrive - Bank Of Israel",
                                 "\\Data\\TASE liquidity",
                                 "\\michael files",
                                 "\\delisted_comps.csv")
...
temp = matching_table_by_size %>% 
  left_join(delisted_comps,
            by = c("tase_id_delisted" = "tase_id")) %>% 
  mutate(dates_seq = map2(ipo_date, delisting_date,
                          function(ipo_date, delisting_date){
                            
                            dates_seq = seq.Date(from = as_date(ipo_date),
                                                 to = as_date(delisting_date),
                                                 by = "quarter")
                            
                            dates_seq = as.yearqtr(dates_seq)
                            
                            return(dates_seq)
   
                            
                          })) %>% 
  select(starts_with("tase_id"), dates_seq) %>% 
  unnest(dates_seq)
