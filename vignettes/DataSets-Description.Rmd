...
A dataset that was gathered by Nimrod . The data time period is `r paste0(range(stata_df$date), collapse = "-")` at quarterly frequency.
...
fin_rep_vars = c("operating_profit","net_profit","equity","revenue",
                 "total_assets","total_liabilities","capex_cashflow",
                 "operating_cashflow","capex_to_revenue","roa",
                 "free_cashflow")

fin_comparison_df = list(finrep_df %>% 
                       select(any_of(c(fin_rep_vars,
                                       "date",
                                       "tase_id"))) %>% 
