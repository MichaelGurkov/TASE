...
raw_oracle_market_df = read_rds(paste0(Sys.getenv("USERPROFILE"),
                          "\\OneDrive - Bank Of Israel\\Data",
                          "\\TASE liquidity\\Rdata files",
...
price_jumps_comps = raw_oracle_market_df %>% 
...
temp_df = raw_oracle_market_df %>% 
  filter(tase_id == "1550") %>% 
  arrange(date) %>% 
  ggplot(aes(date, close)) + 
  geom_line()

raw_oracle_market_df %>% 
  filter(tase_id == "1550") %>% 
  arrange(date) %>% 
