...
style_ret = market_df %>% 
  ...
  filter(days_diff >= 0 & days_diff <= 365 * 3) %>% 
...
temp_first_market_price_date_ipo = min(temp_price_df_ipo$date)

temp_last_market_price_date_ipo = max(which((temp_price_df_ipo$date - min(temp_price_df_ipo$date)) <= 365 * 3))

temp_last_market_price_date_ipo = temp_price_df_ipo$date[temp_last_market_price_date_ipo]

temp_ret_ipo = temp_price_df_ipo %>% 
  filter(date %in% c(temp_first_market_price_date_ipo,
                     temp_last_market_price_date_ipo)) %>% 
  summarise(ret = c(close[2] / close[1] - 1) / 3) %>% 
  pull(ret)
...
temp_first_market_price_date_control = temp_first_market_price_date_ipo

temp_last_market_price_date_control = max(which((temp_price_df_control$date - temp_first_market_price_date_ipo <= 365 * 3)))

temp_last_market_price_date_control = temp_price_df_control$date[temp_last_market_price_date_control]

temp_ret_control = temp_price_df_control %>% 
  filter(date %in% c(temp_first_market_price_date_control,
                     temp_last_market_price_date_control)) %>% 
  summarise(ret = c(close[2] / close[1] - 1) / 3) %>% 
  pull(ret)
