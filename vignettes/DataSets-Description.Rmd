...
merged_comps_df = ipo_comps %>% 
 full_join(delisted_comps, by = "tase_id") %>% 
  filter(ipo_date <= delisting_date) %>% 
  arrange(ipo_date)
...
matching_table_filepath = paste0("C:\\Users\\internet",
                                 "\\OneDrive - Bank Of Israel",
                                 "\\Data\\TASE liquidity",
                                 "\\michael files",
                                 "\\matching_table_by_size.csv")
...
matching_table_by_size = read_csv(matching_table_filepath,
                                  col_types = "ccc",
                                 show_col_types = FALSE)
...
rm(matching_table_filepath,
   delisted_comps_filepath,
   delisted_comps)
...
delisted_df = df %>% 
  inner_join(matching_table_by_size %>% 
               select(tase_id_delisted),
             by = c("tase_id" = "tase_id_delisted"))


matched_df = delisted_df %>% 
  filter(tase_id == 1083) %>% 
  pivot_longer(-c(tase_id, date)) %>% 
  left_join(matching_table_by_size %>% 
              select(starts_with("tase_id")),
            by = c("tase_id" = "tase_id_delisted")) %>% 
  left_join(control_df %>% 
              pivot_longer(-c(tase_id_control, date)),
            by = c("tase_id_control","date","name"),
            suffix = c("_delisted","_control"))

rm(control_df,delisted_df)
...
```{r plot_missing_structure}

matched %>% 
  map_dbl(., ~sum(is.na(.)) / length(.)) %>% 
  as_tibble() %>% 
  mutate(col_names = names(df)) %>% 
  ggplot(aes(value, reorder(col_names, value))) + 
  geom_col() + 
  scale_x_continuous(labels = scales::percent_format()) + 
  xlab(NULL) + ylab(NULL) + ggtitle("Missing values share")
  


```

```{r plot_missing_structure_by_year}
