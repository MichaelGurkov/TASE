...
## Best IPO's price trajectory

```{r best_ipos}


price_df %>% 
  select(id, month, close) %>% 
  filter(month <= 36) %>% 
  group_by(id) %>% 
  arrange(month) %>% 
  mutate(close = close / close[month == 1]) %>% 
  ungroup() %>% 
  ggplot(aes(month, close)) + 
  geom_line(aes(color = id), show.legend = FALSE) + 
  gghighlight::gghighlight(max(close) > 3 & min(close) >= 0.8) +
  geom_hline(yintercept = 1, linetype = "dashed") + 
  xlab("Months since IPO") + ylab(NULL) + ggtitle("IPO price trajectory")
...
# all.equal(temp_price_1212, price_df %>%
#   filter(id == "1212_1091065") %>%
...
temp_plot_df %>% 
  ...
  mutate(category = fct_relevel(category,"price")) %>% 
  ggplot(aes(month, value, color = name)) + 
  geom_line() + 
  facet_wrap(~category, scales = "free") + 
  xlab(NULL) + ylab(NULL) + ggtitle("1212 performance") + 
...
hold_ret %>% 
  ...
  summarise(tibble(quantiles = seq(0.05,0.95,0.05),
                   returns = quantile(hold_ret, seq(0.05,0.95,0.05),
                                      na.rm = TRUE)), .groups = "drop") %>% 
  ggplot(aes(as.character(quantiles), returns, fill = adjustment_type)) + 
  geom_col(position = "dodge") + 
  scale_y_continuous(labels = scales::percent_format()) + 
  ylab(NULL) + xlab("Quantiles") + 
  ggtitle("Distribution of 3 years holding returns")
