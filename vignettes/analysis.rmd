...
results = results %>% 
  mutate(car = map(price_df, function(temp_price_df){
    
    car_horizons = map_dfr(c(1,3,5), function(temp_horizon){
      
      temp_car = temp_price_df %>% 
        calculate_car_from_price_df(target_duration = 252 * temp_horizon,
                                     duration_tolerance = 0.02) %>% 
        mutate(horizon = temp_horizon)
...
car_returns = results %>%
  select(benchmark, car) %>%
  unnest(car) %>%
  group_by(benchmark, horizon, id) %>%
  summarise(car_abnormal = mean(car_abnormal), .groups = "drop_last") %>% 
  mutate(n_comps = length(unique(id))) %>%
  ungroup()
...
```{r sanity_check_bhar_index,eval=FALSE}
...
```{r sanity_check_car_calculation, eval=FALSE}

manual_car_index = results %>% 
  filter(benchmark == "index") %>% 
  select(price_df) %>% 
  unnest(cols = c(price_df)) %>% 
  filter(id == 1240) %>% 
  ...
  arrange(date) %>% 
  mutate(across(contains("price"), ~. / dplyr::lag(.) - 1)) %>% 
  rename_with(~str_replace(.,"price","car")) %>% 
  filter(complete.cases(.)) %>% 
  mutate(car_abnormal = car_ipo - car_control) %>% 
  group_by(id) %>% 
  summarise(car_abnormal = sum(car_abnormal), .groups = "drop")
...
cumret_df_target = results %>% 
  filter(benchmark == "index") %>%
  select(benchmark, price_df) %>% 
  mutate(car_df = map(price_df, function(temp_price_df){

    first_day_df = temp_price_df %>% 
      group_by(id, id_control) %>% 
      filter(date == min(date)) %>% 
      ungroup() %>% 
      mutate(month = 0) %>% 
      select(id,id_control,month,price_ipo,price_control)

    months_df = temp_price_df %>% 
      group_by(id, id_control) %>% 
      filter(!date == min(date)) %>% 
      mutate(month = as.numeric(date - min(date)) %/% 21 + 1) %>% 
      ungroup() %>% 
      filter(month <= 60) %>% 
      group_by(id, id_control, month) %>% 
      summarise(across(contains("price"), mean), .groups = "drop")
    ...
    ret_df = first_day_df %>% 
      ...
      group_by(id, id_control) %>% 
      arrange(month) %>% 
      mutate(across(contains("price"), ~ . / dplyr::lag(.,1) - 1)) %>% 
      ungroup() %>% 
      rename_with(~str_remove_all(.,"price_")) %>% 
      filter(complete.cases(.)) %>% 
      mutate(abnormal = ipo - control) %>% 
      group_by(id, month) %>% 
      arrange(month) %>% 
      mutate(across(c("ipo","abnormal"), cumsum))
  ...
  select(benchmark, car_df) %>% 
  unnest(c(benchmark, car_df))


cumret_df_target %>% 
  filter(id == 1062) %>% 
  select(month, ipo, abnormal) %>% 
  pivot_longer(-month) %>% 
...
car_plot %>% 
  select(benchmark, car_df) %>% 
  unnest(c(benchmark, car_df)) %>% 
  group_by(benchmark, month) %>% 
  summarise(across(c("ipo", "abnormal"), mean)) %>% 
  pivot_longer(-c(benchmark, month)) %>% 
...
```{r sanity_check_car_monthly_calc}

temp_price_df = results$price_df[results$benchmark == "index"][[1]] %>% 
  group_by(id) %>% 
...
months_df = months_df %>% 
  rename_with(~str_remove(., "price_")) %>% 
  group_by(id) %>% 
  arrange(month) %>% 
  mutate(across(c("ipo","control"), ~. / dplyr::lag(.,1) - 1)) %>% 
  ungroup() %>% 
  filter(complete.cases(.)) %>% 
  mutate(abnormal = ipo - control)
