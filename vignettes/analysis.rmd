...
results = list(index = index_match_table,
  ...
  nest(matching_table = c(id_ipo,id_control))
...
```{r save_bhar_results, eval=FALSE}
...
```{r plot_bhar_index_dist, eval=FALSE}
...
```{r plot_bhar_size_dist, eval=FALSE}
...
```{r plot_bhar_book_to_market_dist, eval=FALSE}
...
```{r plot_bhar_by_year, eval=FALSE}
...
cumret_df_target = results %>% 
  ...
  mutate(cumret_df = map(price_df, function(temp_price_df){
    
    temp_cumret_df = temp_price_df %>% 
      convert_to_trading_months() %>% 
      filter(date <= 60) %>% 
      calculate_car_from_price_df()
    
    return(temp_cumret_df)
...
```{r save_monthly_car, eval=FALSE}

cumret_df_target %>% 
  select(benchmark, cumret_df) %>% 
  unnest(c(benchmark, cumret_df)) %>% 
  group_by(benchmark, date) %>% 
  summarise(across(c(cum_ret_ipo,cum_ret_abnormal), mean),
            .groups = "drop") %>%
  write_csv(paste0(Sys.getenv("USERPROFILE"),
                   "\\OneDrive - Bank Of Israel\\research",
                   "\\tase\\article_digest\\monthly_car_plot.csv"))
