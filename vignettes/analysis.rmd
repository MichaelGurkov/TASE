...
results %>% select(benchmark, bhar) %>% 
  unnest(bhar) %>% 
  group_by(benchmark, horizon) %>% 
  summarise(bhar_abnormal = mean(bhar_abnormal), .groups = "drop") %>% 
  pivot_wider(names_from = benchmark, values_from = bhar_abnormal) %>% 
