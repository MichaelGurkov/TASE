...
```{r Import_international_exchanges}


wfe_df = read_csv(paste0("C:\\Users\\internet\\OneDrive - Bank Of Israel\\Data",
                     "\\TASE liquidity\\WFE\\Number_of_listed_comps.csv"))

wfe_df = wfe_df %>% 
  filter(ExchangeName %in% c("Japan Exchange Group Tokyo",
                             "Deutsche Boerse AG",
                             ...
                             "LSE Group")) %>% 
  filter(`Indicator Name` == "Total Equity Market - Number of listed companies (Total)") %>% 
  select(-`Indicator Name`, -Region,-CurrencyName,-DataType) %>% 
