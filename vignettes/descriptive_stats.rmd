...
mcap_by_years = read_csv(paste0(Sys.getenv("USERPROFILE"),
  ...
  left_join(cpi_df %>% 
              ...
              summarise(cpi = mean(cpi), .groups = "drop"), by = "year") %>% 
  mutate(cpi_2019 = cpi / cpi[year == 2019]) %>% 
  mutate(cpi_2022 = cpi / cpi[year == 2022]) %>% 
  mutate(mcap_2019_prices = mcap_2019_prices / cpi_2019) %>% 
  mutate(mcap_2022_prices = mcap_2022_prices / cpi_2019) %>% 
  select(-contains("cpi")) %>% 
  pivot_longer(-year, values_to = "market_value") %>% 
  group_by(date = year) %>% 
  summarise(market_value = mean(market_value, na.rm = TRUE), .groups = "drop")
...
```{r plot_size_quantiles, eval=FALSE}
